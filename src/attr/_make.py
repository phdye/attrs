...
class _Nothing(enum.Enum if hasattr(enum, "Enum") else object):
    """Sentinel to indicate the lack of a value when ``None`` is ambiguous."""

    if hasattr(enum, "auto"):
        NOTHING = enum.auto()
    else:  # pragma: no cover - fallback for Python < 3.4
        NOTHING = object()
...
def _linecache_and_compile(
    ...
    while True:
        ...
        filename = "{0}-{1}>".format(base_filename[:-1], count)
        count += 1
...
def _make_attr_tuple_class(cls_name: str, attr_names: list[str]) -> type:
    ...
    """
    attr_class_name = "{0}Attributes".format(cls_name)
    body = {}
    ...
    for a in (a for a in attrs if a.init is not False and a.kw_only is False):
        if had_default is True and a.default is NOTHING:
            msg = (
                ...
                "default value or factory.  Attribute in question: {0!r}".format(
                    a
                )
            )
            raise ValueError(msg)
...
def evolve(*args, **changes):
    ...
    *changes* applied.
    ...
    try:
        ...
        msg = "evolve() takes 1 positional argument, but {0} were given".format(
            len(args)
        )
...
class _ClassBuilder:
    ...
    def __repr__(self):
        return "<_ClassBuilder(cls={0})>".format(self._cls.__name__)
    ...
    def _add_method_dunders_unsafe(self, method: Callable) -> Callable:
        ...
        method.__qualname__ = "{0}.{1}".format(
            self._cls.__qualname__, method.__name__
        )

        method.__doc__ = "Method generated by attrs for class {0}.".format(
            self._cls.__qualname__
        )
...
def _generate_unique_filename(cls: type, func_name: str) -> str:
    ...
    """
    return (
        "<attrs generated {0} {1}.{2}>".format(
            func_name, cls.__module__, getattr(cls, "__qualname__", cls.__name__)
        )
    )
    ...
    def append_hash_computation_lines(prefix, indent):
        ...
        method_lines.extend(
            [
                ...
                indent + "        {0},".format(type_hash),
            ]
        )

        for a in attrs:
            if a.eq_key:
                cmp_name = "_{0}_key".format(a.name)
                globs[cmp_name] = a.eq_key
                method_lines.append(
                    indent + "        {0}(self.{1}),".format(cmp_name, a.name)
                )
            else:
                method_lines.append(indent + "        {0},".format("self.{0}".format(a.name)))

        method_lines.append(indent + "    " + closing_braces)

    if cache_hash:
        method_lines.append(tab + "if self.{0} is None:".format(_HASH_CACHE_FIELD))
        if frozen:
            append_hash_computation_lines(
                "object.__setattr__(self, '{0}', ".format(_HASH_CACHE_FIELD),
                tab * 2,
            )
            method_lines.append(tab * 2 + ")")  # close __setattr__
        else:
            append_hash_computation_lines(
                "self.{0} = ".format(_HASH_CACHE_FIELD),
                tab * 2,
            )
        method_lines.append(tab + "return self.{0}".format(_HASH_CACHE_FIELD))
    else:
    ...
    if attrs:
        ...
        for a in attrs:
            if a.eq_key:
                ...
                globs[cmp_name] = a.eq_key
                lines.append(
                    "        {0}(self.{1}) == {0}(other.{1})".format(cmp_name, a.name)
                )
            else:
                lines.append(
                    "        self.{0} == other.{0}".format(a.name)
                )
            if a is not attrs[-1]:
                lines[-1] = "{0} and".format(lines[-1])
        lines.append("    )")
    else:
